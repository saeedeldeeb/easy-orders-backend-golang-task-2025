...
func (s *orderService) GetUserOrders(ctx context.Context, userID string, req ListOrdersRequest) (*ListOrdersResponse, error) {
	...
	if err != nil {
		s.logger.Error("Failed to get user for orders", "error", err, "user_id", userID)
		return nil, err
	}
	if user == nil {
		return nil, errors.New("user not found")
	...
	s.logger.Debug("User orders retrieved successfully", "user_id", userID, "count", len(orderResponses))
